...
def generate_pyecharts_sunburst_data(df, matching_docs):
    ...
    # Custom color scheme with BAT blue as the base
    colors = ['#a287ff', '#9295ff', '#83a3ff', '#74b1ff', '#66beff']
...
def create_pyecharts_sunburst_html(data):
    ...
    """
    # Custom background color (light blue tint)
    bg_color = '#ebf2ff'
...
def display_yearly_chart(pub_df):
    ...
    fig = px.bar(
        ...
        color_discrete_sequence=['#0069ce']
    )
...
def create_ingredients_chart(ingredients_data):
    ...
    fig.update_layout(
        ...
        height=500,
        hovermode='closest',
        plot_bgcolor='#ebf2ff'  # White background for a cleaner look
    )
...
def display_sankey_dropdown(categories_dict, tab_title, height=500, bg_color="#f7faff", right="15%"):
    """
    ...
    with st.expander(f"View Source Data for Generated {tab_title} Insights", expanded=False):
        ...
        root_color = '#c4ddf2'  # Soft pink for root
        root_id = "root"
...
def display_main_category_sankey(categories_df, selected_main_category, height=500, bg_color="#f7faff", right="15%"):
    """
